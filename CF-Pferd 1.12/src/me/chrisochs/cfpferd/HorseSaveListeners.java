		...
		if ((e.getRightClicked() instanceof Horse) && e.getPlayer().getItemInHand().getType() == Material.NAME_TAG) {
			...
			try {
				if (h.getCustomName().equalsIgnoreCase("RIDING")
						|| e.getPlayer().getItemInHand().getItemMeta().getDisplayName().equalsIgnoreCase("riding")) {
					e.setCancelled(true);
	...
	public void onPlayerQuit(PlayerQuitEvent e) {
		if (e.getPlayer() instanceof Player) {
			...
			if (p.getVehicle() != null
					&& (p.getVehicle() instanceof /* Horse */ZombieHorse || p.getVehicle() instanceof Boat)) {
		...
		if (e.getEntity() instanceof Player) {
			...
			if (p.getVehicle() instanceof Horse || p.getVehicle() instanceof Boat) {
				if (p.getVehicle().getCustomName() != null
						&& p.getVehicle().getCustomName().equalsIgnoreCase("RIDING")) {
					p.leaveVehicle();
				}
			}
		}
